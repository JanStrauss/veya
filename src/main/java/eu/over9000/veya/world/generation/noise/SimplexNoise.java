...
package eu.over9000.veya.world.generation.noise;
...
public class SimplexNoise { // Simplex noise in 2D, 3D and 4D
	...
	private static final short[] p = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26,
			...
			119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104, 218, 246, 97, 228, 251, 34, 242, 193,
	...
	public static double noise(final double xin, final double yin) {
		...
		final int ii = i & 255;
		final int jj = j & 255;
		final int gi0 = SimplexNoise.permMod12[ii + SimplexNoise.perm[jj]];
		final int gi1 = SimplexNoise.permMod12[ii + i1 + SimplexNoise.perm[jj + j1]];
		final int gi2 = SimplexNoise.permMod12[ii + 1 + SimplexNoise.perm[jj + 1]];
	...
	public static double noise(final double xin, final double yin, final double zin) {
		...
		final int gi1 = SimplexNoise.permMod12[ii + i1 + SimplexNoise.perm[jj + j1 + SimplexNoise.perm[kk + k1]]];
		final int gi2 = SimplexNoise.permMod12[ii + i2 + SimplexNoise.perm[jj + j2 + SimplexNoise.perm[kk + k2]]];
		final int gi3 = SimplexNoise.permMod12[ii + 1 + SimplexNoise.perm[jj + 1 + SimplexNoise.perm[kk + 1]]];
