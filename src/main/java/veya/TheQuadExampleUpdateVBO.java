...
public class TheQuadExampleUpdateVBO {
	...
	private void setupQuad() {
		...
		this.vertices = new TexturedVertex[] { v0, v1, v2, v3 };
		
		// Create a FloatBufer of the appropriate size for one vertex
		this.vertexByteBuffer = BufferUtils.createByteBuffer(TexturedVertex.stride);
		
		// Put each 'Vertex' in one FloatBuffer
		this.verticesByteBuffer = BufferUtils.createByteBuffer(this.vertices.length * TexturedVertex.stride);
		final FloatBuffer verticesFloatBuffer = this.verticesByteBuffer.asFloatBuffer();
		for (final TexturedVertex vertice : this.vertices) {
	...
	private void logicCycle() {
		...
		for (int i = 0; i < this.vertices.length; i++) {
			final TexturedVertex vertex = this.vertices[i];
			
			// Define offset
			final float offsetX = (float) (Math.cos(Math.PI * Math.random()) * 0.1);
			final float offsetY = (float) (Math.sin(Math.PI * Math.random()) * 0.1);
			...
			final FloatBuffer vertexFloatBuffer = this.vertexByteBuffer.asFloatBuffer();
			vertexFloatBuffer.rewind();
			vertexFloatBuffer.put(vertex.getElements());
			vertexFloatBuffer.flip();
			
			GL15.glBufferSubData(GL15.GL_ARRAY_BUFFER, i * TexturedVertex.stride, this.vertexByteBuffer);
